...
def get_ipv6_address():
    ipconfig     = subprocess.check_output(["ipconfig"]).decode().split('\n')
    ipv6_address = None
    
    for line in ipconfig:
        if ("Temporary IPv6 Address" in line):
            ipv6_address = line.split(':', 1)[1].replace(' ', '').replace('\r', '')

    return(ipv6_address)

def update_ddns(ipv6_address):
    ddns_update_response = subprocess.check_output(["curl",server_config["ddns-api-url"].replace("<ip>", ipv6_address)]).decode()
    print("domains.google.com says: " + ddns_update_response)
